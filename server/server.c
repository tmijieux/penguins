 ...
 */
static int move_is_valid_aux(int tile, int direction, int nb_move)
{
    // arguments valides
    if (tile < 0
        || tile > server.tile_count
...
{
    return map.get_number_directions(tile, server.dimension, server.tile_count);
}
...
static void server_compute_neighbours(void)
{
	...
	struct successor_iterator *si = graph_get_successor_iterator(server.graph, i);
	int j = 0;
...
void server_display_directions(int tile_number, int dimension)
{
	...
	int nb_dir = map.get_number_directions(i, dimension,server.tile_count);
...
void server_init(int tile_number, int max_fish,
		 int dimension, const char *map_module_name)
{
    ...
    map.init_graph(dimension, tile_number, server.graph);
...
{
    int isolated = 1;
    struct successor_iterator *vsi = graph_get_successor_iterator(server.graph, tile);
