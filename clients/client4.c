...
static int client_place_penguin(void)
{
    ...
    while (!ok) {
        ...
        int ret = GameM.get_mouseclick_from_display(&mc);
...
{
    int max_dir = GameM.get_tile_nb_direction(src);
    int ok = 0;
    for (int i = 0; !ok && i < max_dir; i++) {
    int dst = 1;
    int j = 1;
    while (dst > 0) {
        dst = GameM.move_is_valid(src, i, j);
        if (dst == trg) {
        *dir = i;
        *jmp = j;
        ok = 1;
...
static void client_play(struct move *retmov)
{
    ...
    while (!ok) {
    struct mouseclick mc;
    int err = GameM.get_mouseclick_from_display(&mc);
    if (err == TC_DISPLAY_THREAD_STOP || err == TC_SURRENDER) {
        GameM.set_move(retmov, -1, -1, -1);
        return;
    }
    ...
    GameM.set_move(retmov, src, dir, jmp);
